...
export function renderTable(state){
  ...
  parents.forEach(p=>{
    ...
    toggle.onclick=()=>{ expandState[p]=!expandState[p]; renderTable(state) }
    ...
    ic.onclick=()=>{ const nv=prompt('Set emoji for '+p+':', state.icons[p]||''); if(nv){ state.icons[p]=nv; renderTable(state) } }
    ...
    tag.onclick=()=>{ state.tags[p]=(state.tags[p]==='F'?'V':'F'); renderTable(state) }
    const add=document.createElement('span'); add.className='chip'; add.textContent='+'; add.title='Add subcategory'
    add.onclick=()=>{ const s=prompt('New subcategory under '+p+':'); if(!s) return; MODEL[p][s]=0; state.order.forEach(k=>{ const mm=state.months[k]; mm.budget[p][s]=0; mm.actual[p][s]=0; }); renderTable(state) }
    const del=document.createElement('span'); del.className='chip'; del.textContent='−'; del.title='Delete parent'
    del.onclick=()=>{ if(!confirm('Delete parent '+p+'?')) return; delete MODEL[p]; delete state.icons[p]; delete state.tags[p]; state.order.forEach(k=>{ const mm=state.months[k]; delete mm.budget[p]; delete mm.actual[p]; }); renderTable(state) }
    ...
    if(expandState[p]){
      Object.keys(MODEL[p]).forEach(s=>{
        ...
        sd.onclick=()=>{ if(!confirm('Delete '+s+'?')) return; delete MODEL[p][s]; state.order.forEach(k=>{ const mm=state.months[k]; delete mm.budget[p][s]; delete mm.actual[p][s]; }); renderTable(state) }
        t0.appendChild(sd)
        tr.appendChild(t0)

        const t1=document.createElement('td'); t1.className='num'; t1.appendChild(numInput(state, key, p, s, 'budget', (m.budget[p]||{})[s]||0)); tr.appendChild(t1)
        const t2=document.createElement('td'); t2.className='num'; t2.appendChild(numInput(state, key, p, s, 'actual', (m.actual[p]||{})[s]||0)); tr.appendChild(t2)
...
function numInput(state, key, p, s, kind, val){
